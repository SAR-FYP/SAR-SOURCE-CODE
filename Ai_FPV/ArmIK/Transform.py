...
#Distance from the origin of the robotic arm, pan-tilt center, to camera image center. The unit is cm. 
...
#convert the pixel coordinate of the image into robotic arm coordinate system
#import coordinate and image resolution. For example, (100, 100, (640, 320))
...
#convert the actual length into image pixel length
#import coordinate and image resolution. For example,(10, (640, 320))
...
# acquire roi area of the detected object
# pass in four vertices returned by cv2.boxPoints(rect). Return extreme point  
def getROI(box):
...
#The area apart form roi turns black
#pass in image, roi area, graphic resolution
...
# acquire center coordinate of block 
# pass in the rect object returned by the minAreaRect function, the extreme point of the block, the image resolution, and the side length of the block
def getCenter(rect, roi, size, square_length):
    x_min, x_max, y_min, y_max = roi
    #According to the coordinates of block center, the vertex closest to the center of the image is selected as the benchmark for calculating the accurate center
    ...
    #convert length into pixel length
    square_l = world2pixel(square_l, size)

    #calculate the center according to the rotation angle of the block
...
# parameter: coordinate of the end of robotic arm, rotation angle of block
