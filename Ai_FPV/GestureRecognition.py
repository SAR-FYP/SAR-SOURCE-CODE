...
def find_contours(binary, max_area):
    ...
    CV_CHAIN_APPROX_TC89_KCOS - application Teh-Chin approximation algorithm. CV_LINK_RUNS - Using a completely different contour extraction algorithm through the horizontal fragment connected to 1
...
def get_hand_number(binary_image, contours, rw, rh, rgb_image):
    ...
    if contours is not None:
        ...
        # cv2.polylines(rgb_image, [approx], True, (0, 255, 0), 1)#draw the polygon
        ...
        if approx.shape[0] >= 3:  # there are more than three points#The smallest polygon is triangle. Triangle requires three points.
            ...
            for i in range(1, len(approx_list) - 1):
                ...
                line2 = Line(p2, p3)
                angle = GetCrossAngle(line1, line2)     # acquire the angle between two straight lines
                angle = 180 - angle     #
                # print angle
...
def run(img, debug=False):
    ...
    cv2.putText(img, "Finger(s):%d" % __finger, (50, 480 - 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 255), 2)#write the number of recognized finger on the picture
